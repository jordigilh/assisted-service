...
var _ = Describe("Refresh Host", func() {
	...
	Context("Validate host", func() {
		tests := []struct {
			...
			},
			{
				...
				hostID:        strfmt.UUID("054e0100-f50e-4be7-874d-73861179e40d"),
				inventory:     hostutil.GenerateInventoryWithResourcesWithBytes(1, hardware.GibToBytes(17), "worker"),
				role:          models.HostRoleWorker,
				srcState:      models.HostStatusDiscovering,
				dstState:      models.HostStatusInsufficient,
	...
	Context("All transitions", func() {
		...
		tests := []struct {
			...
			{
				name:               "OCS enabled: known to insufficient CPU and memory with 3 master hosts",
				validCheckInTime:   true,
				srcState:           models.HostStatusKnown,
				dstState:           models.HostStatusInsufficient,
				machineNetworkCidr: "1.2.3.0/24",
				...
				role:               models.HostRoleMaster,
				statusInfoChecker: makeValueChecker(formatStatusInfoFailedValidation(statusInfoNotReadyForInstall,
					"Require at least 14 CPU cores for master role, found only 8", "Require at least 41 GiB RAM role master, found only 16 GiB")),
				inventory: hostutil.GenerateMasterInventory(),
				validationsChecker: makeJsonChecker(map[validationID]validationCheckResult{
					IsConnected:                 {status: ValidationSuccess, messagePattern: "Host is connected"},
					HasInventory:                {status: ValidationSuccess, messagePattern: "Valid inventory exists for the host"},
					...
					HasMinValidDisks:            {status: ValidationSuccess, messagePattern: "Sufficient disk capacity"},
					IsMachineCidrDefined:        {status: ValidationSuccess, messagePattern: "Machine Network CIDR is defined"},
					HasCPUCoresForRole:          {status: ValidationFailure, messagePattern: "Require at least 14 CPU cores for master role, found only 8"},
					HasMemoryForRole:            {status: ValidationFailure, messagePattern: "Require at least 41 GiB RAM role master, found only 16 GiB"},
					IsHostnameUnique:            {status: ValidationSuccess, messagePattern: "is unique in cluster"},
					BelongsToMachineCidr:        {status: ValidationSuccess, messagePattern: "Host belongs to machine network CIDR"},
					IsNTPSynced:                 {status: ValidationSuccess, messagePattern: "Host NTP is synced"},
					AreContainerImagesAvailable: {status: ValidationSuccess, messagePattern: "All required container images were pulled and are available"},
				...
				errorExpected:      false,
				operators:          []*models.MonitoredOperator{&ocs.Operator, &lso.Operator},
				numAdditionalHosts: 2,
			},
			{
				name:               "OCS enabled: known to known with sufficient CPU and memory with 3 master",
				validCheckInTime:   true,
				srcState:           models.HostStatusKnown,
				dstState:           models.HostStatusKnown,
				machineNetworkCidr: "1.2.3.0/24",
				...
				inventory:          hostutil.GenerateInventoryWithResources(14, 42, "master-1"),
				validationsChecker: makeJsonChecker(map[validationID]validationCheckResult{
					IsConnected:                 {status: ValidationSuccess, messagePattern: "Host is connected"},
					HasInventory:                {status: ValidationSuccess, messagePattern: "Valid inventory exists for the host"},
					HasMinCPUCores:              {status: ValidationSuccess, messagePattern: "Sufficient CPU cores"},
			...
			{
				name:               "OCS enabled: known to insufficient CPU with 3 master hosts",
				validCheckInTime:   true,
				srcState:           models.HostStatusKnown,
				dstState:           models.HostStatusInsufficient,
				machineNetworkCidr: "1.2.3.0/24",
				...
				role:               models.HostRoleMaster,
				statusInfoChecker: makeValueChecker(formatStatusInfoFailedValidation(statusInfoNotReadyForInstall,
					"Require at least 14 CPU cores for master role, found only 8")),
				inventory: hostutil.GenerateInventoryWithResources(8, 42, "master-1"),
				validationsChecker: makeJsonChecker(map[validationID]validationCheckResult{
					IsConnected:                 {status: ValidationSuccess, messagePattern: "Host is connected"},
					HasInventory:                {status: ValidationSuccess, messagePattern: "Valid inventory exists for the host"},
					HasMinCPUCores:              {status: ValidationSuccess, messagePattern: "Sufficient CPU cores"},
			...
			{
				name:               "OCS enabled: insufficient to known with 6 worker hosts and not enough memory",
				validCheckInTime:   true,
				srcState:           models.HostStatusInsufficient,
				dstState:           models.HostStatusInsufficient,
				machineNetworkCidr: "1.2.3.0/24",
				...
				role:               models.HostRoleWorker,
				statusInfoChecker: makeValueChecker(formatStatusInfoFailedValidation(statusInfoNotReadyForInstall,
					"Require at least 14 GiB RAM role worker, found only 8 GiB")),
				inventory: hostutil.GenerateInventoryWithResources(8, 8, "worker-1"),
				validationsChecker: makeJsonChecker(map[validationID]validationCheckResult{
					IsConnected:                 {status: ValidationSuccess, messagePattern: "Host is connected"},
					HasInventory:                {status: ValidationSuccess, messagePattern: "Valid inventory exists for the host"},
					HasMinCPUCores:              {status: ValidationSuccess, messagePattern: "Sufficient CPU cores"},
					...
					IsMachineCidrDefined:        {status: ValidationSuccess, messagePattern: "Machine Network CIDR is defined"},
					HasCPUCoresForRole:          {status: ValidationSuccess, messagePattern: "Sufficient CPU cores for role worker"},
					HasMemoryForRole:            {status: ValidationFailure, messagePattern: "Require at least 14 GiB RAM role worker, found only 8 GiB"},
					IsHostnameUnique:            {status: ValidationSuccess, messagePattern: "is unique in cluster"},
					BelongsToMachineCidr:        {status: ValidationSuccess, messagePattern: "Host belongs to machine network CIDR"},
					IsNTPSynced:                 {status: ValidationSuccess, messagePattern: "Host NTP is synced"},
					AreContainerImagesAvailable: {status: ValidationSuccess, messagePattern: "All required container images were pulled and are available"},
				...
				errorExpected:      false,
				operators:          []*models.MonitoredOperator{&ocs.Operator, &lso.Operator},
				numAdditionalHosts: 6,
			},
			{
				name:               "OCS enabled: insufficient to known with 6 worker hosts and not enough CPU",
				validCheckInTime:   true,
				srcState:           models.HostStatusInsufficient,
				dstState:           models.HostStatusInsufficient,
				machineNetworkCidr: "1.2.3.0/24",
				...
				role:               models.HostRoleWorker,
				statusInfoChecker: makeValueChecker(formatStatusInfoFailedValidation(statusInfoNotReadyForInstall,
					"Insufficient CPU count to deploy OCS in Minimal Deployment Mode. A minimum of 18 cores is needed")),
				inventory: hostutil.GenerateInventoryWithResources(2, 16, "worker-1"),
				validationsChecker: makeJsonChecker(map[validationID]validationCheckResult{
					IsConnected:                 {status: ValidationSuccess, messagePattern: "Host is connected"},
					HasInventory:                {status: ValidationSuccess, messagePattern: "Valid inventory exists for the host"},
					HasMinCPUCores:              {status: ValidationSuccess, messagePattern: "Sufficient CPU cores"},
					...
					HasMinValidDisks:            {status: ValidationSuccess, messagePattern: "Sufficient disk capacity"},
					IsMachineCidrDefined:        {status: ValidationSuccess, messagePattern: "Machine Network CIDR is defined"},
					HasCPUCoresForRole:          {status: ValidationFailure, messagePattern: "Insufficient CPU count to deploy OCS in Minimal Deployment Mode. A minimum of 18 cores is needed"},
					HasMemoryForRole:            {status: ValidationSuccess, messagePattern: "Sufficient RAM for role worker"},
					IsHostnameUnique:            {status: ValidationSuccess, messagePattern: "is unique in cluster"},
					BelongsToMachineCidr:        {status: ValidationSuccess, messagePattern: "Host belongs to machine network CIDR"},
					IsNTPSynced:                 {status: ValidationSuccess, messagePattern: "Host NTP is synced"},
			...
			{
				name:               "LSO enabled: known to known with 1 worker and minimum HW resources",
				validCheckInTime:   true,
				srcState:           models.HostStatusKnown,
				dstState:           models.HostStatusKnown,
				machineNetworkCidr: "1.2.3.0/24",
				...
				errorExpected:      false,
				operators:          []*models.MonitoredOperator{&lso.Operator},
				numAdditionalHosts: 0,
			},
			{
				name:               "LSO enabled: known to known with sufficient CPU and memory with 3 master",
				validCheckInTime:   true,
				srcState:           models.HostStatusKnown,
				dstState:           models.HostStatusKnown,
				machineNetworkCidr: "1.2.3.0/24",
				...
				role:               models.HostRoleMaster,
				statusInfoChecker:  makeValueChecker(formatStatusInfoFailedValidation(statusInfoKnown)),
				inventory:          hostutil.GenerateInventoryWithResources(4, 16, "master-1"),
				validationsChecker: makeJsonChecker(map[validationID]validationCheckResult{
					IsConnected:                 {status: ValidationSuccess, messagePattern: "Host is connected"},
					HasInventory:                {status: ValidationSuccess, messagePattern: "Valid inventory exists for the host"},
					HasMinCPUCores:              {status: ValidationSuccess, messagePattern: "Sufficient CPU cores"},
			...
			{
				name:               "CNV + OCS enabled: known to insufficient with lack of CPU and memory with 3 workers",
				validCheckInTime:   true,
				srcState:           models.HostStatusKnown,
				dstState:           models.HostStatusInsufficient,
				machineNetworkCidr: "1.2.3.0/24",
				...
				role:               models.HostRoleWorker,
				statusInfoChecker: makeValueChecker(formatStatusInfoFailedValidation(statusInfoNotReadyForInstall,
					"Insufficient hosts to deploy OCS: Exactly 3 master hosts are required to deploy OCS in compact mode",
					"Unable to determine host memory requirements: Insufficient hosts to deploy OCS: Exactly 3 master hosts are required to deploy OCS in compact mode")),
				inventory: hostutil.GenerateInventoryWithResources(12, 12, "worker-1"),
				validationsChecker: makeJsonChecker(map[validationID]validationCheckResult{
					IsConnected:                 {status: ValidationSuccess, messagePattern: "Host is connected"},
					HasInventory:                {status: ValidationSuccess, messagePattern: "Valid inventory exists for the host"},
					HasMinCPUCores:              {status: ValidationSuccess, messagePattern: "Sufficient CPU cores"},
					...
					HasMinValidDisks:            {status: ValidationSuccess, messagePattern: "Sufficient disk capacity"},
					IsMachineCidrDefined:        {status: ValidationSuccess, messagePattern: "Machine Network CIDR is defined"},
					HasCPUCoresForRole:          {status: ValidationFailure, messagePattern: "Insufficient hosts to deploy OCS: Exactly 3 master hosts are required to deploy OCS in compact mode"},
					HasMemoryForRole:            {status: ValidationFailure, messagePattern: "Insufficient hosts to deploy OCS: Exactly 3 master hosts are required to deploy OCS in compact mode"},
