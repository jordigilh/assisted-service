...
func (m *MetricsManager) NetworkLatencyBetweenHosts(clusterVersion string, sourceRole, targetRole models.HostRole, latency float64) {
	m.serviceLogicNetworkLatencyMilliseconds.WithLabelValues(clusterVersion, string(sourceRole), string(targetRole)).Observe(latency)
}
func (m *MetricsManager) PacketLossBetweenHosts(clusterVersion string, sourceRole, targetRole models.HostRole, packetLoss float64) {
	m.serviceLogicNetworkLatencyMilliseconds.WithLabelValues(clusterVersion, string(sourceRole), string(targetRole)).Observe(packetLoss)
}
